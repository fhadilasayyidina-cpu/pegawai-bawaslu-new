<div>
    <x-header-page title="Detail Pegawai" :breadcrumbs="[
        ['label' => 'Admin', 'href' => '#'],
        ['label' => 'Pegawai', 'href' => '/admin/pegawais'],
        ['label' => 'Detail'],
    ]">
        <x-slot:actions>
            <x-mary-button label="Cuti" icon="o-calendar" link="/admin/cutis/create?pegawai_id={{ $pegawai->id }}"
                variant="secondary" />
            <flux:button variant="ghost" icon="calendar-days" href="/admin/pegawais/{{ $pegawai->id }}/absensis">
                Lihat Absensi
            </flux:button>
        </x-slot:actions>
    </x-header-page>
    ...
    <x-mary-tabs wire:model="selectedTab" selected="ringkasan-tab">
                ...
                <div class="lg:col-span-1">
                    <x-mary-card>
                        <div class="flex flex-col items-center p-6">
                            ...
                            <flux:badge variant="secondary" class="mt-2">
                                Id Absensi : {{ $pegawai->id_absensi ?: '-' }}
                            </flux:badge>
                        </div>
                    </x-mary-card>
                </div>
                        ...
                        <x-mary-card>
                            <div class="flex items-center gap-3">
                                <div class="p-3 bg-orange-100 dark:bg-orange-900 rounded-lg">
                                    <flux:icon name="calendar-date-range"
                                ...
                                <div>
                                    ...
                                    @if ($pegawai->tgl_kgb_terakhir)
                                        <flux:heading size="sm">
                                            {{ $pegawai->tgl_kgb_terakhir->translatedFormat('d F Y') }}
                                        </flux:heading>
                                    @else
                        ...
                        <x-mary-card>
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    @if ($this->nextKgbDate)
                                        <flux:heading size="sm">
                                            {{ $this->nextKgbDate->translatedFormat('d F Y') }}
                                        </flux:heading>
            ...
            <x-mary-form wire:submit="saveIdentitas">
                ...
                <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div class="flex items-start gap-6">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            @else
                                <flux:avatar :name="$pegawai->nama" size="lg"
                        ...
                        <div class="flex-1">
                            ...
                            <flux:input type="file" wire:model="identitasForm.foto" accept="image/*"
                                class="mb-2" />

                            @if ($pegawai->foto)
                                <flux:button variant="danger" size="sm" wire:click="deleteFoto">
                                    Hapus Foto
                                </flux:button>
                            @endif
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <flux:input label="Tanggal Lahir" type="date" wire:model="identitasForm.tgl_lahir"
                        description="{{ $pegawai->tgl_lahir ? $pegawai->tgl_lahir->translatedFormat('d F Y') : '' }}" />
                ...
                <x-slot:actions>
                    <x-mary-button label="Simpan Identitas" type="submit" class="btn-primary"
                        spinner="saveIdentitas" />
                </x-slot:actions>
            </x-mary-form>
        </x-mary-tab>
        ...
        <x-mary-tab name="jabatan-tab" label="Jabatan & Golongan" icon="o-briefcase">
            <x-mary-form wire:submit="saveJabatan">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <flux:input label="Link Google Drive SK Golongan Awal"
                            wire:model="jabatanForm.sk_golongan_awal_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->sk_golongan_awal_drive_link)
                            <a href="{{ $pegawai->sk_golongan_awal_drive_link }}" target="_blank"
                    ...
                    <div>
                        <flux:input label="Link Google Drive SK Golongan Terakhir"
                            wire:model="jabatanForm.sk_golongan_terakhir_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->sk_golongan_terakhir_drive_link)
                            <a href="{{ $pegawai->sk_golongan_terakhir_drive_link }}" target="_blank"
                                rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                    ...
                    <flux:input label="TMT Golongan" type="date" wire:model="jabatanForm.tmt_golongan"
                        description="{{ $pegawai->tmt_golongan ? $pegawai->tmt_golongan->translatedFormat('d F Y') : '' }}" />
                    ...
                    <div>
                        <flux:input label="Link Google Drive SK Jabatan"
                            wire:model="jabatanForm.sk_jabatan_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->sk_jabatan_drive_link)
                            <a href="{{ $pegawai->sk_jabatan_drive_link }}" target="_blank"
                                rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                    ...
                    <flux:input label="TMT Jabatan" type="date" wire:model="jabatanForm.tmt_jabatan"
                        description="{{ $pegawai->tmt_jabatan ? $pegawai->tmt_jabatan->translatedFormat('d F Y') : '' }}" />
                ...
                <x-slot:actions>
                    <x-mary-button label="Simpan Jabatan" type="submit" class="btn-primary"
                        spinner="saveJabatan" />
                </x-slot:actions>
            </x-mary-form>
        </x-mary-tab>
        ...
        <x-mary-tab name="administrasi-tab" label="Administrasi" icon="o-document">
            <x-mary-form wire:submit="saveAdministrasi">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <flux:input label="Link Google Drive NPWP" wire:model="administrasiForm.npwp_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->npwp_drive_link)
                            <a href="{{ $pegawai->npwp_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link NPWP
                            </a>
                    ...
                    <div>
                        <flux:input label="Link Google Drive BPJS" wire:model="administrasiForm.bpjs_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->bpjs_drive_link)
                            <a href="{{ $pegawai->bpjs_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link BPJS
                            </a>
                    ...
                    <div>
                        <flux:input label="Link Google Drive Karpeg" wire:model="administrasiForm.karpeg_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->karpeg_drive_link)
                            <a href="{{ $pegawai->karpeg_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link Karpeg
                            </a>
                    ...
                    <div>
                        <flux:input label="Link Google Drive SK CPNS" wire:model="administrasiForm.sk_cpns_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->sk_cpns_drive_link)
                            <a href="{{ $pegawai->sk_cpns_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link SK CPNS
                            </a>
                    ...
                    <flux:input label="Tanggal SK CPNS" type="date" wire:model="administrasiForm.tgl_sk_cpns"
                        description="{{ $pegawai->tgl_sk_cpns ? $pegawai->tgl_sk_cpns->translatedFormat('d F Y') : '' }}" />
                    <flux:input label="TMT CPNS" type="date" wire:model="administrasiForm.tmt_cpns"
                        description="{{ $pegawai->tmt_cpns ? $pegawai->tmt_cpns->translatedFormat('d F Y') : '' }}" />
                    ...
                    <div>
                        ...
                        @if ($pegawai->sk_pns_drive_link)
                            <a href="{{ $pegawai->sk_pns_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link SK PNS
                            </a>
                    ...
                    <flux:input label="Tanggal SK PNS" type="date" wire:model="administrasiForm.tgl_sk_pns"
                        description="{{ $pegawai->tgl_sk_pns ? $pegawai->tgl_sk_pns->translatedFormat('d F Y') : '' }}" />
                    <flux:input label="TMT PNS" type="date" wire:model="administrasiForm.tmt_pns"
                        description="{{ $pegawai->tmt_pns ? $pegawai->tmt_pns->translatedFormat('d F Y') : '' }}" />
                    ...
                    <flux:input label="Tanggal SK DPK" type="date"
                        ...
                        description="{{ $pegawai->tgl_sk_dpk_penugasan_kontrak ? $pegawai->tgl_sk_dpk_penugasan_kontrak->translatedFormat('d F Y') : '' }}" />
                    ...
                    <flux:input label="Tanggal KGB Terakhir" type="date"
                        ...
                        description="{{ $pegawai->tgl_kgb_terakhir ? $pegawai->tgl_kgb_terakhir->translatedFormat('d F Y') : '' }}" />
                    <div>
                        <flux:input label="Link Google Drive SK KGB" wire:model="administrasiForm.sk_kgb_drive_link"
                            placeholder="https://drive.google.com/..." />
                        @if ($pegawai->sk_kgb_drive_link)
                            <a href="{{ $pegawai->sk_kgb_drive_link }}" target="_blank" rel="noopener noreferrer"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 mt-1">
                                <flux:icon name="link" class="w-4 h-4" />
                                Buka Link SK KGB
                            </a>
                ...
                <x-slot:actions>
                    <x-mary-button label="Simpan Administrasi" type="submit" class="btn-primary"
                        spinner="saveAdministrasi" />
                </x-slot:actions>
            </x-mary-form>
        </x-mary-tab>
        ...
        <x-mary-tab name="pendidikan-tab" label="Pendidikan" icon="o-academic-cap">
            <x-mary-form wire:submit="savePendidikan">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-mary-input label="Pendidikan Tertinggi"
                        wire:model="pendidikanForm.pendidikan_tertinggi_nama" />
                    ...
                    <x-mary-textarea label="Riwayat Diklatpim" wire:model="pendidikanForm.riwayat_diklatpim"
                        rows="3" class="md:col-span-2" />
                </div>

                <x-slot:actions>
                    <x-mary-button label="Simpan Pendidikan" type="submit" class="btn-primary"
                        spinner="savePendidikan" />
                </x-slot:actions>
            </x-mary-form>
        </x-mary-tab>
        ...
        <x-mary-tab name="unit-organisasi-tab" label="Unit & Organisasi" icon="o-building-office">
            <x-mary-form wire:submit="saveUnitOrganisasi">
                ...
                <x-slot:actions>
                    <x-mary-button label="Simpan Unit & Organisasi" type="submit" class="btn-primary"
                        spinner="saveUnitOrganisasi" />
                </x-slot:actions>
            </x-mary-form>
        </x-mary-tab>
    </x-mary-tabs>
